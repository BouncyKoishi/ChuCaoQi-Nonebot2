...
起卦_cmd = on_command('起卦', priority=5, block=True)


@起卦_cmd.handle()
async def handle_起卦(bot: Bot, event: Event, args: Message = CommandArg()):
    global gptUseRecord, trigramResults
    user_id = await get_user_id(event)
    stripped_arg = args.extract_plain_text().strip()
    
    if len(stripped_arg) > 100:
        await send_finish(起卦_cmd, '暂不支持过长的起卦内容^ ^')
        return
    
    hashing_str = stripped_arg + str(user_id) + str(time.strftime("%Y-%m-%d %H", time.localtime())) + 'Trigram'
    random.seed(hash(hashing_str))
    ...
    output_str = f'对【{stripped_arg}】的占卜结果为：\n' if stripped_arg != '' else '占卜结果为：\n'
    output_str += f'爻：{getSymbolsName(symbols, changeableIndex)} '
    output_str += f'({getSymbolsSign(outerTrigram8, innerTrigram8, changedOuterTrigram8, changedInnerTrigram8)})\n'
    output_str += f'本卦：{getTrigramName(outerTrigram8, innerTrigram8, trigram64)}   '
    output_str += f'之卦：{getTrigramName(changedOuterTrigram8, changedInnerTrigram8, changedTrigram64)}\n' if len(changeableIndex) != 0 else ''
    output_str += f'卦辞：{getFinalWords(trigram64, changedTrigram64, changeableIndex)}'
    output_str += '\n如果需要进一步解析，请输入"!解卦"'
    
    random.seed()
    
    trigramResults[user_id] = {
        'trigram64': trigram64,
        'changedTrigram64': changedTrigram64,
        'changeableIndex': changeableIndex,
        'stripped_arg': stripped_arg,
    }
    
    await send_finish(起卦_cmd, output_str)


解卦_cmd = on_command('解卦', priority=5, block=True)


@解卦_cmd.handle()
async def handle_解卦(bot: Bot, event: Event, args: Message = CommandArg()):
    global gptUseRecord, trigramResults
    user_id = await get_user_id(event)
    
    if user_id not in trigramResults:
        await send_finish(解卦_cmd, '请先使用"起卦"命令进行占卜。')
        return
    
    if gptUseRecord.get(user_id, 0) > 10:
        await send_finish(解卦_cmd, '今日解卦次数已达上限，请明日再来。')
        return
    
    gptUseRecord[user_id] = gptUseRecord.get(user_id, 0) + 1
    await send_reply(解卦_cmd, '请稍等，正在为你解卦...')
    
    result = trigramResults[user_id]
    ...
    reply_msg = (await ChatService.get_chat_reply("deepseek-flash", chat_gpt_prompt, source="trigrams")).reply
...
def getTrigram64(linearSymbols):
    trigramsMap = {
        ...
        b'111001': {'order': 25, 'base': '无妄：元亨利贞。其匪正有眚，不利有攸往。', 'line1': '初九：无妄，往吉。', 'line2': '六二：不耕获，不菑畲，则利有攸往。', 'line3': '六三：无妄之灾，或系之牛，行人的得，邑人的灾。', 'line4': '九四：可贞，无咎。', 'line5': '九五：无妄之疾，勿药有喜。', 'line6': '上九：无妄，行有眚，无攸利。'},
        ...
        b'111100': {'order': 33, 'base': '遁：亨，小利贞。', 'line1': '初六：遁尾，厉，勿用有攸往。', 'line2': '六二：执之用黄牛的革，莫之胜说。', 'line3': '九三：系遁，有疾厉，畜臣妾，吉。', 'line4': '九四：好遁，君子吉，小人否。', 'line5': '九五：嘉遁，贞吉。', 'line6': '上九：肥遁，无不利。'},
        ...
        b'010101': {'order': 63, 'base': '既济：亨，小利贞，初吉终乱。', 'line1': '初九：曳其轮，濡其尾，无咎。', 'line2': '六二：妇丧其茀，勿逐，七日得。', 'line3': '九三：高宗伐鬼方，三年克之，小人勿用。', 'line4': '六四：繻有衣袽，终日戒。', 'line5': '九五：东邻杀牛，不如西邻的禴祭，实受其福。', 'line6': '上六：濡其首，厉。'},
