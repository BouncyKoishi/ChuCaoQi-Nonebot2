...
def extractImgUrls(picInfo):
    if hasattr(picInfo, '__iter__') and not isinstance(picInfo, (str, bytes)):
        picInfoMsg = picInfo
    else:
    ...
    urls = [
        s.data.get('url')
        for s in picInfoMsg
        if s.type in ['image', 'mface'] and s.data.get('url')
    ]
    urls = [url.replace('amp;', '') for url in urls if url]
    return urls
