...
export const ALL_CARDS: CardData[] = [
  ...
  // 红魔乡 - 琪露诺
  { id: 29, cost: 1, name: '雹符「Hailstorm」', cardHp: 5, atkPoint: '1d7', defPoint: '1', dodPoint: '1d2', description: '宣言时：造成1点伤害', character: 'cirno', onCardSet(u, e) { const info = e.effectHurt(1); return `[${u.name}]冰雹暴风！造成1点伤害\n${info}` } },
  ...
  // 红魔乡 - 帕秋莉
  { id: 35, cost: 3, name: '火符「Agni Radiance」', cardHp: 6, atkPoint: '1d8', defPoint: '1', dodPoint: '1d2', description: '宣言时：造成3点伤害', character: 'patchouli', onCardSet(u, e) { const info = e.effectHurt(3); return `[${u.name}]火神的光辉！造成3点伤害\n${info}` } },
  ...
  { id: 37, cost: 3, name: '木符「Green Storm」', cardHp: 7, atkPoint: '1d5', defPoint: '1d3', dodPoint: '1d3', description: '每回合开始时：展开2回合的[迟缓结界1]', character: 'patchouli', onTurnStart(u, _e) { u.appendBorder('SluggishBorder', 2, 1); return `[${u.name}]绿色风暴！\n` } },
  ...
  { id: 40, cost: 3, name: '火&土符「Lava Cromlech」', cardHp: 7, atkPoint: '1d6', defPoint: '1d3', dodPoint: '1d2', description: '宣言时：造成2点伤害；获得[缓冲1]', character: 'patchouli', onCardSet(u, _e) { const info = u.enemy!.effectHurt(2); u.appendEffect('Buffer', 1); return `[${u.name}]熔岩环石阵！\n${info}` } },
  ...
  { id: 46, cost: 5, name: '日符「Royal Flare」', cardHp: 7, atkPoint: '2d8', defPoint: '1', dodPoint: '1', description: '宣言时：造成4点伤害；每回合开始时：获得[弱化1]', character: 'patchouli', onCardSet(u, e) { const info = e.effectHurt(4); return `[${u.name}]皇家烈焰！造成4点伤害\n${info}` }, onTurnStart(u, _e) { u.appendEffect('Weaken', 1); return `[${u.name}]烈焰反噬！获得弱化1\n` } },
  ...
  { id: 52, cost: 4, name: '奇术「Eternal Meek」', cardHp: 6, atkPoint: '1d3', defPoint: '1d3', dodPoint: '1d3', description: '每回合开始时：造成1d4点伤害；获得[追击1d3]', character: 'sakuya', onTurnStart(u, e) { const rng = new SeededRandom(); const dmg = rollDice('1d4', rng); const chaseAmt = rollDice('1d3', rng); const info = e.effectHurt(dmg); u.appendEffect('Chase', chaseAmt); return `[${u.name}]永恒的谦卑！造成${dmg}点伤害，获得追击${chaseAmt}\n${info}` } },
  // 红魔乡 - 蕾米莉亚
  ...
  { id: 63, cost: 5, name: '秘弹「之后就一个人都没有了吗？」', cardHp: 3, atkPoint: '1d3', defPoint: '1d3', dodPoint: '1d3', description: '[时符3]；每回合开始时：造成2点伤害', character: 'flandre', isTimeCard: true, timeCardTurns: 3, onTurnStart(u, e) { const info = e.effectHurt(2); return `[${u.name}]无人生还！造成2点伤害\n${info}` } },
