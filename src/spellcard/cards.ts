...
export const ALL_CARDS: CardData[] = [
  ...
  {
    id: 7, cost: 2, name: '「人偶振起」', cardHp: 4, atkPoint: '1', defPoint: '0', dodPoint: '1d2+1',
  ...
  { id: 22, cost: 4, name: '红符「深红射击」', cardHp: 2, atkPoint: '2d10', defPoint: '1', dodPoint: '1', description: '无' },
  {
  ...
  // 红魔乡 - 琪露诺
  { id: 29, cost: 1, name: '雹符「Hailstorm」', cardHp: 5, atkPoint: '1d7', defPoint: '1', dodPoint: '1d2', description: '宣言时：造成1点伤害', character: 'cirno', onCardSet(u, e) { e.effectHurt(1); return `[${u.name}]冰雹暴风！造成1点伤害\n` } },
  { id: 30, cost: 2, name: '雪符「Diamond Blizzard」', cardHp: 6, atkPoint: '1d5', defPoint: '1d3', dodPoint: '1d3', description: '宣言时：展开3回合的[迟缓结界1]', character: 'cirno', onCardSet(u, e) { u.appendBorder('SluggishBorder', 3, 1); return `[${u.name}]钻石风暴！\n` } },
  // 红魔乡 - 红美铃
  ...
  { id: 34, cost: 2, name: '彩符「极彩台风」', cardHp: 7, atkPoint: '2d4', defPoint: '1d3', dodPoint: '1d2', description: '宣言时：对方获得[破甲1]', character: 'meiling', onCardSet(_u, e) { e.appendEffect('CantDefence', 1, '破甲'); return `[${_u.name}]台风！对方获得破甲1\n` } },
  // 红魔乡 - 帕秋莉
  { id: 35, cost: 3, name: '火符「Agni Radiance」', cardHp: 6, atkPoint: '1d8', defPoint: '1', dodPoint: '1d2', description: '宣言时：造成3点伤害', character: 'patchouli', onCardSet(u, e) { e.effectHurt(3); return `[${u.name}]火神的光辉！造成3点伤害\n` } },
  ...
  { id: 37, cost: 3, name: '木符「Green Storm」', cardHp: 7, atkPoint: '1d5', defPoint: '1d3', dodPoint: '1d3', description: '回合开始时：展开2回合的[迟缓结界1]', character: 'patchouli', onTurnStart(u, _e) { u.appendBorder('SluggishBorder', 2, 1); return `[${u.name}]绿色风暴！\n` } },
  ...
  { id: 40, cost: 3, name: '火&土符「Lava Cromlech」', cardHp: 7, atkPoint: '1d6', defPoint: '1d3', dodPoint: '1d2', description: '宣言时：造成2点伤害；获得[缓冲1]', character: 'patchouli', onCardSet(u, _e) { u.enemy!.effectHurt(2); u.appendEffect('Buffer', 1); return `[${u.name}]熔岩环石阵！\n` } },
  ...
  { id: 42, cost: 3, name: '水&木符「Water Elf」', cardHp: 8, atkPoint: '1d4', defPoint: '1d3', dodPoint: '1d3', description: '回合开始时：获得[灵动1]；回合结束时：失去[灵动1]', character: 'patchouli', onTurnStart(u, _e) { u.appendEffect('Agile', 1); return `[${u.name}]水精灵之舞！\n` }, onTurnEnd(u, _e) { u.removeEffect('Agile', 1); return '' } },
  ...
  { id: 46, cost: 5, name: '日符「Royal Flare」', cardHp: 7, atkPoint: '2d8', defPoint: '1', dodPoint: '1', description: '宣言时：造成4点伤害；回合开始时：获得[弱化1]', character: 'patchouli', onCardSet(u, e) { e.effectHurt(4); return `[${u.name}]皇家烈焰！造成4点伤害\n` }, onTurnStart(u, _e) { u.appendEffect('Weaken', 1); return `[${u.name}]烈焰反噬！获得弱化1\n` } },
  { id: 47, cost: 5, name: '火水木金土符「贤者之石」', cardHp: 12, atkPoint: '1d6', defPoint: '1d4', dodPoint: '1d3', description: '回合开始时：获得[强化1][稳固1]；回合结束时：获得[弱化1]', character: 'patchouli', onTurnStart(u, _e) { u.appendEffect('Strength', 1); u.appendEffect('Stable', 1); return `[${u.name}]贤者之石！获得强化1、稳固1\n` }, onTurnEnd(u, _e) { u.appendEffect('Weaken', 1); return `[${u.name}]贤者之石的代价！获得弱化1\n` } },
  // 红魔乡 - 咲夜
  ...
  { id: 50, cost: 4, name: '幻世「The World」', cardHp: 8, atkPoint: '1d6', defPoint: '1d3', dodPoint: '1d2', description: '宣言时：赋予对方[时停1]；获得[强化2]', character: 'sakuya', onCardSet(u, e) { e.appendEffect('Freeze', 1, '时停'); u.appendEffect('Strength', 2); return `[${u.name}]THE WORLD！对方获得时停1，自身获得强化2\n` } },
  { id: 51, cost: 3, name: '女仆秘技「杀人玩偶」', cardHp: 6, atkPoint: '1d4', defPoint: '1d2', dodPoint: '1d2', description: '宣言时：获得[连击3]；回合开始时：获得[强化1]', character: 'sakuya', onCardSet(u, _e) { u.appendEffect('Combo', 3); return `[${u.name}]杀人玩偶！获得连击3\n` }, onTurnStart(u, _e) { u.appendEffect('Strength', 1); return `[${u.name}]飞刀越打越痛！获得强化1\n` } },
  { id: 52, cost: 4, name: '奇术「Eternal Meek」', cardHp: 6, atkPoint: '1d3', defPoint: '1d3', dodPoint: '1d3', description: '回合开始时：造成1d4点伤害；获得[追击1d3]', character: 'sakuya', onTurnStart(u, e) { const rng = new SeededRandom(); const dmg = rollDice('1d4', rng); const chaseAmt = rollDice('1d3', rng); e.effectHurt(dmg); u.appendEffect('Chase', chaseAmt); return `[${u.name}]永恒的谦卑！造成${dmg}点伤害，获得追击${chaseAmt}\n` } },
  // 红魔乡 - 蕾米莉亚
  ...
  { id: 56, cost: 5, name: '红符「Scarlet Meister」', cardHp: 8, atkPoint: '2d6', defPoint: '1d2', dodPoint: '1', description: '宣言时：获得[吸血2][追击2]', character: 'remilia', onCardSet(u, _e) { u.appendEffect('Drain', 2); u.appendEffect('Chase', 2); return `[${u.name}]绯红之主！获得吸血2、追击2\n` } },
  ...
  { id: 59, cost: 4, name: '禁忌「Laevatein」', cardHp: 6, atkPoint: '2d8', defPoint: '0', dodPoint: '1', description: '宣言时：对方获得[贯穿1]', character: 'flandre', onCardSet(_u, e) { e.appendEffect('CantDefence', 1, '贯穿'); return `[${_u.name}]莱瓦汀！对方获得贯穿1\n` } },
  { id: 60, cost: 4, name: '禁忌「恋之迷宫」', cardHp: 8, atkPoint: '1d5', defPoint: '1d3', dodPoint: '1d3', description: '宣言时：对方获得[迟缓2][回避不可1]', character: 'flandre', onCardSet(_u, e) { e.appendEffect('Sluggish', 2); e.appendEffect('CantDodge', 1); return `[${_u.name}]恋之迷宫！\n` } },
  { id: 61, cost: 4, name: '禁弹「Catadioptric」', cardHp: 7, atkPoint: '1d5', defPoint: '1d3', dodPoint: '1d2', description: '宣言时：获得[荆棘1]；回合开始时：获得[追踪1]', character: 'flandre', onCardSet(u, _e) { u.appendEffect('Thorns', 1); return `[${u.name}]折反射！获得荆棘1\n` }, onTurnStart(u, _e) { u.appendEffect('Trace', 1); return `[${u.name}]弹幕反射！获得追踪1\n` } },
  { id: 62, cost: 5, name: '禁弹「刻着过去的钟表」', cardHp: 9, atkPoint: '1d5', defPoint: '1d3', dodPoint: '1d2', description: '回合开始时：获得[强化1][稳固1]', character: 'flandre', onTurnStart(u, _e) { u.appendEffect('Strength', 1); u.appendEffect('Stable', 1); return `[${u.name}]时钟滴答！获得强化1、稳固1\n` } },
  { id: 63, cost: 5, name: '秘弹「之后就一个人都没有了吗？」', cardHp: 3, atkPoint: '1d3', defPoint: '1d3', dodPoint: '1d3', description: '[时符3]；回合开始时：造成2点伤害', character: 'flandre', isTimeCard: true, timeCardTurns: 3, onTurnStart(u, e) { e.effectHurt(2); return `[${u.name}]无人生还！造成2点伤害\n` } },
