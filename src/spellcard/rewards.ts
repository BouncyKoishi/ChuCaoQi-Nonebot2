...
const EFFECT_POOL: EffectModule[] = [
  {
    ...
    description: '宣言时对敌方造成1点直接伤害（无视防御与闪避）',
  ...
  },
  {
    ...
    description: '宣言时获得追击（对方受伤时额外+1伤害）',
  ...
  },
  {
    ...
    description: '宣言时获得追踪（对方闪避成功时仍受1点伤害）',
  ...
  },
  {
    ...
    description: '宣言时对敌方造成2点直接伤害（无视防御与闪避）',
    apply(user: Battler, enemy: Battler) {
  ...
  {
    id: 'set_freeze', displayName: '宣言·冻结', slot: 'onCardSet', rarity: 'rare',
    description: '宣言时冻结对方1回合（对方无法攻击/防御/闪避）',
    apply(_user: Battler, enemy: Battler) { enemy.appendEffect('Freeze', 1); return '' },
  },
  {
    id: 'set_cantdef', displayName: '宣言·防御不可', slot: 'onCardSet', rarity: 'rare',
    description: '宣言时对方无法防御（受伤=攻击力，无减伤）',
    apply(_user: Battler, enemy: Battler) { enemy.appendEffect('CantDefence', 1); return '' },
  },
  {
    id: 'set_cantdod', displayName: '宣言·回避不可', slot: 'onCardSet', rarity: 'rare',
    description: '宣言时对方无法闪避（闪避率归零）',
  ...
  },
  {
    ...
    description: '被击破时对敌方造成1点直接伤害（无视防御与闪避）',
    apply(user: Battler, enemy: Battler) {
      const info = enemy.effectHurt(1)
  ...
  {
    id: 'set_dmg2', displayName: '宣言·直伤2', slot: 'onCardSet', rarity: 'rare',
    description: '宣言时对敌方造成2点直接伤害',
    apply(user: Battler, enemy: Battler) {
      const info = enemy.effectHurt(2)
      return `[${user.name}]宣言直伤，造成2点直接伤害\n` + info
    },
  },
  {
  ...
  },
  {
    ...
    description: '宣言时获得连击（对方符卡已受伤时攻击力+1）',
  ...
  {
    id: 'turn_thorns1', displayName: '被动·荆棘', slot: 'onPassive', rarity: 'rare',
    description: '受到战斗伤害时反弹1点伤害',
    apply(user: Battler, _enemy: Battler) { user.removeEffect('Thorns', 1); user.appendEffect('Thorns', 1); return '' },
  },
]
...
export function generateRewards(
  ...
  if (battleType === 'boss') {
    ...
    const roll = rng()
    rareCount = roll < 0.3 ? 3 : roll < 0.7 ? 2 : 1
  } else {
    const roll = rng()
    rareCount = roll < 0.05 ? 2 : roll < 0.2 ? 1 : 0
  }
  const commonCount = 3 - rareCount
  ...
  for (let i = 0; i < rareCount; i++) {
    const pool = [rareEffects, rareDice, rareStats, SLOT_POOL]
    let attempts = 0
    while (attempts < 20) {
  ...
  for (let i = 0; i < commonCount; i++) {
    const pool = [commonEffects, commonDice, commonStats]
    let attempts = 0
    while (attempts < 20) {
      const chosen = pool[Math.floor(rng() * pool.length)]
      const item = chosen[Math.floor(rng() * chosen.length)]
